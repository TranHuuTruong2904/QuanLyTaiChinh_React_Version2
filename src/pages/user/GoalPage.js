...
const GoalPage = () => {
  ...
  async function listGoalSearch() {
    const encodedSearchText = encodeURIComponent(searchText);
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL +
        `/api/goal/search?name=${encodedSearchText}`
    );
    if (result?.data?.status === 101) {
      toast.error("Không có mục tiêu cần tìm kiếm");
      setListGoal([]);
    } else {
      setListGoal(result?.data?.data);
    }
  ...
  const handleAddDeposit = async (e) => {
    ...
    try {
      const result = await axiosApiInstance.post(
        axiosApiInstance.defaults.baseURL +
          `/api/goal/add-deposit/${selectedGoal?.id}`,
        payload
      );
  ...
  const handleAddGoal = async () => {
    try {
      const result = await axiosApiInstance.post(
        axiosApiInstance.defaults.baseURL + `/api/goal/add`,
        newGoal
      );
      if (
        result?.data?.message.includes(
          "Mục tiêu với mốc thời gian đã được đặt ra"
        )
      ) {
        toast.error(result?.data?.message);
      } else if (
        result?.data?.message.includes(
          "Mốc thời gian đặt ra mục tiêu phải lớn hơn ngaày hiện tại!"
        )
      ) {
        toast.error(result?.data?.message);
      } else {
  ...
  const handleUpdateGoal = async () => {
    try {
      ...
      const result = await axiosApiInstance.put(
        axiosApiInstance.defaults.baseURL +
          `/api/goal/update/${selectedGoal?.id}`,
        payload
      );
      if (
        result?.data?.message.includes(
        ...
        result?.data?.message.includes(
          "Mốc thời gian đặt ra mục tiêu phải lớn hơn ngày hiện tại!"
        )
      ) {
        toast.error(result?.data?.message);
      } else {
  ...
  async function deleteGoal() {
    try {
      const result = await axiosApiInstance.delete(
        axiosApiInstance.defaults.baseURL +
          `/api/goal/delete/${selectedGoal?.id}`
      );
      if (result?.data?.status === 200) {
  ...
  return (
    <>
      ...
      >
        <div className="d-flex text-muted overflow-auto center">
          <table className="table">
            <thead>
              <tr>
                <th scope="col" className="col-5 col-name">
                  Ngày thêm
                </th>
                <th scope="col" className="col-5 col-name">
                  Số tiền thêm
            ...
            <tbody>
              {selectedGoal?.goalDetailList.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    {item.deposit?.toLocaleString("vi", {
                      style: "currency",
                      currency: "VND",
                    })}
                  </td>
            ...
            <tbody>
              <p>
                Tổng:{" "}
                {selectedGoal?.totalDeposit?.toLocaleString("vi", {
                  style: "currency",
                  currency: "VND",
                })}
              </p>
      ...
      <Modal
        title={<div className="text-center">Thêm tiền cho mục tiêu</div>}
        open={isModalAddDeposit}
        onOk={handleAddDeposit}
        onCancel={handleCancel}
      >
      ...
      <Modal
        title={<div className="text-center">Thêm mục tiêu mới</div>}
        open={isModalAddGoal}
        onOk={handleAddGoal}
        onCancel={handleCancel}
      >
        <Form layout="vertical" autoComplete="off" form={form1}>
      ...
      <Modal
        title={<div className="text-center">Sửa mục tiêu</div>}
        open={isModalUpdateGoal}
      ...
      <Modal
        title={<div className="text-center">Bạn có chắc muốn xóa mục tiêu</div>}
        open={isModalDeleteGoal}
        onOk={deleteGoal}
        onCancel={handleCancel}
      ></Modal>
    </>
