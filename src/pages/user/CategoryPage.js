...
const CategoryPage = () => {
  ...
  const currentUserCategoryList = paginate(
    listUserCategory,
    currentPage,
    pageSize
  );
  ...
  async function getUserCategory() {
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL + `/api/user-category/all`
    );
    setLoad(true);
  ...
  async function listUserCategorySearch() {
    const encodedSearchText = encodeURIComponent(searchText);
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL +
        `/api/user-category/search?name=${encodedSearchText}`
    );
    if (result?.data?.status === 101) {
      toast.error("Không có danh mục cần tìm kiếm");
      setListUserCategory([]);
    } else {
      setListUserCategory(result?.data?.data);
    }
  ...
  const [isModalUpdateUserCategory, setIsModelUpdateUserCategory] =
    useState(false);
  const [isModalDeleteUserCategory, setIsModalDeleteUserCategory] =
    useState(false);
  ...
  const handleAddCategory = async () => {
    try {
      const result = await axiosApiInstance.post(
        axiosApiInstance.defaults.baseURL +
          `/api/user-category/add/${idCategoryType}`,
        newCategory
      );
      if (result?.data?.status === 101) {
  ...
  const handleUpdateCategory = async () => {
    try {
      ...
      const result = await axiosApiInstance.put(
        axiosApiInstance.defaults.baseURL +
          `/api/user-category/update/${selectedCategory?.id}/${idCategoryType}`,
        payload
      );
      if (result?.data?.status === 101) {
        toast.error(result?.data?.message);
  ...
  async function deleteUserCategory() {
    try {
      const result = await axiosApiInstance.delete(
        axiosApiInstance.defaults.baseURL +
          `/api/user-category/delete/${selectedCategory?.id}`
      );
      if (result?.data?.status === 200) {
        ...
        if (
          result?.data?.message.includes(
            "Danh mục đang được sử dụng để lưu trữ giao dịch. Không được phép xóa"
          )
        ) {
          toast.error(result?.data?.message);
          setIsModalDeleteUserCategory(false);
        ...
        if (
          result?.data?.message.includes(
            "Danh mục đang được sử dụng cho một ngân sách. Không được xóa"
          )
        ) {
          toast.error(result?.data?.message);
          setIsModalDeleteUserCategory(false);
  ...
  return (
    <>
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th scope="col" className="col-3 col-name">
                      Tên danh mục
                    </th>
                    <th scope="col" className="col-3 col-name">
                      Ghi chú
      ...
      <Modal
        title={<div className="text-center">Thêm danh mục mới</div>}
        open={isModalAddUserCategory}
      ...
      <Modal
        title={<div className="text-center">Sửa danh mục</div>}
        open={isModalUpdateUserCategory}
      ...
      <Modal
        ...
        open={isModalDeleteUserCategory}
        onOk={deleteUserCategory}
        onCancel={handleCancel}
      ></Modal>
    </>
  );
